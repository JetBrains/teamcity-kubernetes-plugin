...
public class UpgradeArgumentsProvider implements HelmCommandArgumentsProvider {
    ...
    public List<String> getArguments(@NotNull Map<String, String> runnerParameters) {
        String release = runnerParameters.get(RELEASE_NAME);
        String chart = runnerParameters.get(CHART);
        String additionalFlags = runnerParameters.get(ADDITIONAL_FLAGS);
        return StringUtil.splitCommandArgumentsAndUnquote(
          String.format("%s %s %s %s", HELM_UPGRADE_COMMAND,
                        StringUtil.emptyIfNull(release), StringUtil.emptyIfNull(chart), StringUtil.emptyIfNull(additionalFlags)
          ));
    }
}
